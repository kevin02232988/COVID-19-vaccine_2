...
기존 연구([Sun Woong Kim (2021). COVID-19 Fear Index and Stock Market. Journal of Convergence for Information Technology, 11(9), 84-93](https://www.kci.go.kr/kciportal/ci/sereArticleSearch/ciSereArtiView.kci?sereArticleSearchBean.artiId=ART002756191)  : *COVID 부정여론과 공포지수*)는 공포지수와 금융시장 사이의 상관성은 잘 보여주지만,
...
- **데이터**: 코로나/백신 관련 영어권 온라인 댓글 약 **108k건** 크롤링 → 정제 후 **약 23k건** 분석
- **모델**: DeBERTa v3 기반 이진 감성 분류 모델 (**Val. Acc ≈ 0.87**)
- **핵심 결과**
  - 전체 댓글 중 **부정 감성 ≈ 86%**, 긍정 ≈ 14%
  - 부정 비율 시계열 vs 공포·탐욕 지수 → **DTW ≈ 3.44**로 패턴 상당히 유사
  - 부정 토픽의 중심축이 **“부작용”보다 “정책·의무화·경제·정치 갈등”**에 더 가깝게 나타남
- **의미**
  - 온라인 여론 데이터로 **팬데믹 정책·커뮤니케이션 리스크를 정량적으로 모니터링**할 수 있는 가능성을 제시
...
프로젝트의 파이썬 스크립트는 **역할별로** 아래처럼 정리했다.  
...
- `pharmacy_P.py`, `pharmacy_sem.py`, `pharmacy_tai.py`, `pharmacy_total.py`, `Durg_T.py`, `PP.py`  
  - Drugs.com, WebMD, 기타 헬스 관련 사이트 리뷰 크롤링/파싱 스크립트들

> **역할 요약**  
> 여러 사이트에서 코로나/백신 관련 텍스트를 수집하여  
> 최종적으로 **원본 통합 CSV(`FINAL_DATA_CLEANED_READY.csv`)**를 만들기 위한 단계.
...
- `refine.py`  
  - 삭제된 댓글, 너무 짧은 잡담, 비영어 텍스트, URL/특수문자 등  
  - 구조적/형식적 노이즈 제거

- `code for erase.py`  
  - 링크 위주, 의미 없는 중립 텍스트 등  
  - 특정 패턴 삭제용 유틸 스크립트
...
- 병합/통합 스크립트들
  ...
  - `com.py`, `comb.py`, `comb1.py`, `10.py`, `10w.py`, `10per.py` 등

  - 여러 소스에서 온 CSV를 읽어 **컬럼 정렬/통일**
  - **중복 제거, 필터링, 샘플링** 수행
  - 최종 분석용 데이터셋 **`DDDD.csv`**까지 이어지는 **중간 단계 데이터들 생성**

> **역할 요약**  
> 크롤링 결과를  
> **정제 → 관련성 필터링(True/False) → 링크/중립 삭제 → 최종 분석용 데이터**로  
> 단계적으로 좁혀 나가는 전처리·통합 파이프라인.
...
- `label.py`, `rabel.py`, `rabel d.py`, `rebel d.py`  
  ...
  - Binary / Three-Class 라벨을 사람이 직접 입력할 수 있는 CSV 템플릿 생성
...
### 4. 감성 분석 모델 학습  
(Sentiment Modeling: KoELECTRA / BERT / DeBERTa)
...
- `koel_finetuning.py`  
  ...
  - Binary / 3-Class 세팅, 클래스 가중치, 오버샘플링 등 다양한 설정 시도

#### 4.2 BERT / DeBERTa 계열 실험

- DeBERTa 계열 메인 실험 스크립트
  - `debert.py`, `debert 1.py`, `debert 2.py`, `debert 3.py`, `debert 4.py`, `debert 5.py`  
  - `debert 1_1.py`, `debert 1_7.py`, `devert_3.py`

  - DeBERTa / DeBERTa v3 기반 **Binary 감성 분류 모델**  
  - 손실 함수(Focal Loss), 클래스 가중치, 오버샘플링, Epoch 수 등  
    **하이퍼파라미터 조합을 바꿔가며 실험한 버전들**

#### 4.3 학습 로그/성능 분석

- `accu.py`, `anal.py`, `asd.py`, `asdg.py`, `f.py`, `fd.py`, `gr.py`, `gra1.py`, `grh2.py`  
  - 학습 로그에서 Accuracy, Loss, F1-score 등을 읽어와 표/그래프로 정리  
  - 여러 실험 결과를 비교하여  
    **Validation Accuracy ≈ 0.87**인 최종 DeBERTa 모델 선정

> **역할 요약**  
> 여러 모델과 설정을 비교한 뒤  
> 최종적으로 **DeBERTa v3 기반 Binary 감성 분류 모델**을 선택하는 과정.

---

### 5. 토픽 모델링 (Topic Modeling: LDA / BERTopic)

감성 라벨이 붙은 텍스트에서 **어떤 주제(토픽)**가 나오는지 분석하는 스크립트들.

- `topic.py`, `topic_1.py`  
  - 초기 **LDA(Latent Dirichlet Allocation)** 실험  
    - 전처리 문제를 발견하는 데 활용  
    - 경제·정치·마스크·의료비 등 주요 토픽 윤곽 파악  
  - 이후 **BERTopic + HDBSCAN** 기반 토픽 모델링 파이프라인 구현  
    - 토픽별 키워드/Count/Representation 계산  
    - 부정/긍정 리뷰를 분리한 토픽 분석 수행

> **역할 요약**  
> 부정/긍정 여론이  
> **정책·의무화·음모론·의료비 등 어떤 주제를 중심으로 형성되는지**  
> 정성적·정량적으로 해석하는 단계.

---

### 6. 시계열 & 공포·탐욕 지수 결합  
(Time Series & Fear-Greed Index)

날짜별 부정 비율 시계열과  
외부 금융 지표(Fear-Greed Index)를 결합하고  
상관/DTW를 계산하는 스크립트들.

- `13_time include.py`, `14 time include.py`  
  - 감성 라벨이 붙은 데이터(`DDDD.csv`)에서 **날짜별 부정 비율** 계산  
  - `FEAR#.csv`와 날짜 기준으로 merge

- `15 combined.py`, `16 K.py`, `17.py`, `17k.py`, `18_com.py`,  
  `19.py`, `20.py`, `21.py`, `22.py`, `23.py`, `24.py`, `25.py`  
  - Fear-Greed Index와 부정 비율에 대해
    - 피어슨 상관계수  
    - p-value  
...
> 얼마나 동행하는지 정량적으로 검증하는 단계.

---

### 7. 시각화 (Visualization)

분석 결과를 그림으로 만드는 스크립트들  
(논문/보고서 Figure 생성용).

- `13_graph.py`, `22_legend.py`  
  - 시계열 그래프, 막대 그래프, 라벨 분포 등 주요 플롯 생성

- `debertvisu.py`, `debertvisu_1.py`, `debertvisu_2.py`  
  - DeBERTa 학습/예측 결과 시각화  
    (Acc/Loss 곡선, Confusion Matrix 등)

- `dwq.py`, `dwq1.py`, `dwqq.py`, `dwqqq.py`, `dwqqqq.py`, `dwqqqqq.py`, `dwqqqqqq.py`  
  - 그래프 스타일, 레이블, 축 설정 등을 실험한 버전별 스크립트

- `o.py`, `oo.py`, `op.py`, `q.py`, `ra.py`, `run.py`, `run2.py`, `to.py`  
  - 개별 그래프/테스트용 간단 플롯 스크립트들

> **역할 요약**  
> README / 보고서에서 사용한  
> `A.png`, `B.png`, `C.png`, `D.png`, `E.png`, `7.png`, `8.png`, `9.png` 등  
> Figure를 실제로 생성하는 코드들.
...
- `d.py`, `dd.py`, `f.py`, `k.py`, `In_t.py`, `T.py` 등
...
> 실제 파일명은 저장소 구조에 맞게 정리되어 있으며,  
> 여기서는 **역할 단위**로만 소개한다.

- `src/crawling/`  
  - Reddit 및 각종 포럼/리뷰 사이트 크롤러

- `src/preprocessing/`  
  - 전처리 파이프라인  
    (구조적 노이즈 제거, 언어 필터링, 키워드 기반 필터 등)

- `src/labeling/`  
  - 수동 라벨링용 샘플링 스크립트, 라벨 병합 도구

- `src/modeling/`  
  - DeBERTa v3 감성 분류 모델 학습 코드

- `src/topic_modeling/`  
  - LDA, BERTopic 토픽 모델링 스크립트
